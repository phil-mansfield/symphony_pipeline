    ...
    for i_suite in range(len(suites)):
        ...
        for halo_type in range(2):
            ...
            for i_host in range(symlib.n_hosts(suite)):    
                ...
                print(np.sum(ok[:,-1] & (hist["mpeak_pre"] > 1e8)))
                print(np.sum(ok[:,-1] & (ratio > 1e-4)))
...
def rct_cdf_fit(r):
    r = np.log10(r)
    c = -0.14340697*r**4 - 0.61043872*r**3 - 1.67231339*r**2 + 0.6528787*r
    return 10**c
...
def radius_cdf():
    ...
    if SUBFIND_RADIUS_CDF:
        ...
        cut_labels = [
            r"$n_{\rm sub,infall} > 5.9\times 10^{1}$",
            r"$n_{\rm sub,infall} > 4.7\times 10^{2}$",
            r"$n_{\rm sub,infall} > 3.7\times 10^{3}$",
            r"$n_{\rm sub,infall} > 1.3\times 10^{4}$",
            r"$n_{\rm sub,infall} > 1.1\times 10^{5}$"
        ][::-1]
    ...
    fig_suffix = ["main", "hr", "c", "rs"]
    if SUBFIND_RADIUS_CDF:
        fig_labels = [[r"${\rm Particle{-}tracking}$", r"${\rm Subfind}$"],
                      [r"${\rm Particle{-}tracking}$", r"${\rm Subfind}$"],
                ...
                for i_curve in range(len(suites_to_plot)):
                    ...
                    for i_mult in range(len(npeak_cuts)):
                        ...
                        ax.plot(r_plot[size_ok],
                                ...
                                ls=ls, c=color)
                        cdf_fit = rct_cdf_fit(r_plot)
                        rat.plot(r_plot[size_ok],
                                 (np.arange(np.sum(ok))/np.sum(ok)/cdf_fit)[size_ok],
                                 ls=ls, c=color)
